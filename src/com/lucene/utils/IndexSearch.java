...
public class IndexSearch {
	...
	private void doSearch(Query query) {
		...
		try {
			...
			for (ScoreDoc scoreDoc : scoreDocs) {
				...
				System.out.println("书本ID：" + doc.get("bid"));
				System.out.println("书本名称：" + doc.get("bname"));
				System.out.println("书本类型：" + doc.get("btype"));
				System.out.println("书本描述：" + doc.get("description"));
				System.out.println("==========================");
			}
			// 关闭资源
			reader.close();
	...
	public void indexSearch() throws Exception {
		...
		// 第一个参数：默认搜索的域的名称
		QueryParser parser = new QueryParser("bid",
				new StandardAnalyzer());

		// 通过queryparser来创建query对象
		// 参数：输入的lucene的查询语句(关键字一定要大写)
		Query query = parser.parse("bid:1");
